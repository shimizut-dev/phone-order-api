...
GitHub Actions では Debian Trixie Slim コンテナ上で `sh ./mvnw spotless:check test` を実行し、tracked file
がビルド後に変化しないことも検証します。
...
- 品質チェックには Checkstyle によるソースコード構成ルール確認と SpotBugs による Java バグ検出も含む
