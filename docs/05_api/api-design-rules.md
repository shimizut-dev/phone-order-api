...
- 一覧取得 API でページングする場合は `page` と `size` を query parameter として定義する
- `page` は 0 始まり、未指定時は `0` とする
- `size` は 1 ページあたりの取得件数とし、未指定時は `20`、最大値は `100` とする
- ページング済み一覧レスポンスには `items`, `page`, `size`, `totalElements`, `totalPages`, `hasNext`, `hasPrevious` を含める
- 一覧取得 API はソート順を description に明記する
- 総ページ数を超える `page` を指定した場合は、空の `items` と指定ページのページ情報を返す
