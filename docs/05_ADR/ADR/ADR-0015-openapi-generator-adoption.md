...
phone-order-api では、API の外部仕様を `specs/openapi/openapi.yaml` で管理している。
...
API のエンドポイント、リクエスト、レスポンス、エラー構造は `specs/openapi/openapi.yaml` に定義する。
...
- API 仕様変更時は `specs/openapi/openapi.yaml` を先に更新する
- OpenAPI Generator 実行前に仕様変更が `specs/openapi/openapi.yaml` に反映されていることを確認する
