...
phone-order-api では、API の外部仕様を `docs/02_設計/API設計/OpenAPI仕様.yaml` で管理している。
...
`specs/openapi/openapi.yaml` から API インターフェースと API 入出力モデルを生成する。
...
API のエンドポイント、リクエスト、レスポンス、エラー構造は `docs/02_設計/API設計/OpenAPI仕様.yaml` に定義する。
ビルド時は `specs/openapi/openapi.yaml` を OpenAPI Generator の入力として参照する。
...
- API 仕様変更時は `docs/02_設計/API設計/OpenAPI仕様.yaml` を先に更新する
- OpenAPI Generator を実行する前に `specs/openapi/openapi.yaml` へ同じ内容を反映する
- `docs/02_設計/API設計` と README には、生成物を手動編集しない方針を記載する
